...
def download(ctx, segment_id, start_date, end_date, api_key, output_path):
    ...
    if start_date is not None:
        if start_date > end_date:
            sys.exit("The start date must be earlier than the end date.\nAborting.")
    ...
    if not _response_is_healthy(response):
        if response.status_code == 403:
            sys.exit("API key was rejected by the Telraam server.\nAborting.")
        sys.exit("Couldn't reach url " + url + ".\nAborting.")

    if len(response.json()["camera"]) == 0:
        sys.exit("No segment with id " + segment_id + " found.\nAborting.")
    ...
    if df is None:
        sys.exit("An unexpected error happened.\nAborting.")
